...
train = pd.read_csv("data/data_format1/test_format1.csv")
...
user_id:用户名字
merchant_id：商家名字
repeat_buy_user.action_type：‘0’点击、‘1’添加购物车、‘2’购买、‘3’添加收藏夹
"""
repeat_buy_user = pd.read_csv(
    "/Users/junguang/Desktop/data_format1/repeat_buy_user.csv")#打开csv文件
...
x.loc[x.buy_times_between_user_and_seller ==
      5, "user_buy_fivetime_in_this_seller"] = 1
x.loc[x.buy_times_between_user_and_seller ==
      6, "user_buy_sixtime_in_this_seller"] = 1
...
seller_be_repeat_buy_by_how_many_user = seller_be_repeat_buy_by_how_many_user.drop(["user_id", "cat_id", "time_stamp", "brand_id", "action_type"], axis=1)
...
a = a.reset_index()
b = a[a.action_type > 1]  # 过滤掉没有重复购买记录的user_id - seller_id 对
...
a = a[a.action_type == 2] #action_type == 2 表示购买
b = a.groupby(["user_id", "seller_id"]).count()
b = b.reset_index()
buy_times_between_user_and_seller_befor_1111 = pd.DataFrame(#购买次数
...
a = user_log[user_log.time_stamp != 1111]
a = a[a.action_type == 3] #action_type == 2 表示加入收藏夹
b = a.groupby(["user_id", "seller_id"]).count()
b = b.reset_index()
favourite_times_between_user_and_seller_befor_1111 = pd.DataFrame(#收藏次数
...
user_info = pd.read_csv(
    "/Users/junguang/Desktop/data_format1/user_info_format1.csv").reset_index()#user_id,age_range,gender
...
a_ucs = user_log[user_log.action_type == 2].groupby(
    ["user_id", "cat_id", "seller_id"]).count().reset_index()              #"user_id", "cat_id", "seller_id"
...
3           420       1784    2        2   2   881     # 用户1784在商家2购买过类目420下的商品2次
                                                       # 商家2类目420下的商品销售共计881
